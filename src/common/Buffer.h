...
public:
	...
	virtual GVectorEx<TData> * Get(const int iRequestedSize) = 0;
	virtual GVectorEx<TData> * QueryWriteBuffer() = 0;
	virtual void Put(const int iOfferedSize) = 0;
	virtual void Clear() = 0;
	virtual int GetFillLevel() const = 0;

protected:
	GVectorEx<TData> vecBuffer;
//	int iBufferSize;
...
public:
	...
	virtual void Init(const int iNewBufferSize);
	virtual GVectorEx<TData>* Get(const int iRequestedSize);
	virtual GVectorEx<TData>* QueryWriteBuffer() {return &this->vecBuffer;}
...
public:
	...
	virtual void Init(const int iNewBufferSize);
	virtual GVectorEx<TData>* Get(const int iRequestedSize);
	virtual GVectorEx<TData>* QueryWriteBuffer() {return &vecInOutBuffer;}
	virtual void Put(const int iOfferedSize);
	virtual void Clear();
	virtual int GetFillLevel() const;

protected:
	GVectorEx<TData>	vecInOutBuffer;
...
//	iBufferSize = iNewBufferSize;
	...
	vecBuffer.resize(iNewBufferSize);
}
...
template<class TData> GVectorEx<TData>* CSingleBuffer<TData>::Get(const int iRequestedSize)
{
	/* Get data ***************************************************************/
#ifdef _DEBUG_
...
template<class TData> void CCyclicBuffer<TData>::Init(const int iNewBufferSize)
{
	...
	vecInOutBuffer.resize(iNewBufferSize);
...
GVectorEx<TData> * CCyclicBuffer<TData>::Get(const int iRequestedSize)
{
	int	i;
	int	iAvailSpace;
	int iElementCount;
	...
	if ((iAvailSpace <= 0) && (iBufferState == BS_FULL))
		iAvailSpace += this->vecBuffer.size();
	...
	if ((int)this->vecBuffer.size() - iGet < iRequestedSize)
	{
		/* Data must be read in two portions */
		for (i = iGet; i < (int)this->vecBuffer.size(); i++)
		{
			vecInOutBuffer[iElementCount] = this->vecBuffer[i];
			iElementCount++;
		}
		for (i = 0; i < iRequestedSize - (int)this->vecBuffer.size() + iGet; i++)
		{
			vecInOutBuffer[iElementCount] = this->vecBuffer[i];
			iElementCount++;
		}
	...
	/* Adjust iGet pointer */
	iGet += iRequestedSize;
	if (iGet >= (int)this->vecBuffer.size())
		iGet -= this->vecBuffer.size();
...
template<class TData> void CCyclicBuffer<TData>::Put(const int iOfferedSize)
{
	...
	/* Test if wrap is needed */
	if ((iAvailSpace <= 0) && (iBufferState == BS_EMPTY))
		iAvailSpace += this->vecBuffer.size();
	...
	if ((int)this->vecBuffer.size() - iPut < iOfferedSize)
	{
		/* Data must be written in two steps */
		for (i = iPut; i < (int)this->vecBuffer.size(); i++)
		{
			this->vecBuffer[i] = vecInOutBuffer[iElementCount];
			iElementCount++;
		}
		for (i = 0; i < iOfferedSize - (int)this->vecBuffer.size() + iPut; i++)
		{
			this->vecBuffer[i] = vecInOutBuffer[iElementCount];
			iElementCount++;
		}
	...
	/* Adjust iPut pointer */
	iPut += iOfferedSize;
	if (iPut >= (int)this->vecBuffer.size())
		iPut -= this->vecBuffer.size();
...
template<class TData> int CCyclicBuffer<TData>::GetFillLevel() const
{
	...
	if ((iFillLevel == 0) && (iBufferState == BS_FULL))
		iFillLevel = this->vecBuffer.size();
	if (iFillLevel < 0)
		iFillLevel += this->vecBuffer.size();	/* Wrap around */
