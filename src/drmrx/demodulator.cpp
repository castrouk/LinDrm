...
//	extern QLineEdit Callsign;

#include <QColor>
//extern QColor Io_Led;
extern QColor Freq_Led;
//extern QColor Time_Led;
//extern QColor Frame_Led;
//extern QColor FAC_Led;
//extern QColor MSC_Led;
...
//	qDebug() << __FILE__ << __LINE__;
	...
	if (!timeSyncFlag)
	{
		...
		if ((fabsf(samplerate_offset_estimation) > 200.0E-6) && (robustness_mode != 99))
		{
			N_samples_needed = N_symbols_mode_detection * 320;
			input_samples_buffer_request = N_samples_needed;
			rsbufwidx = 0;
...
bool demodulator::frameSync()
{
	...
	if (!frameSyncFlag)
	{
		...
		//      qDebug() << "symbols_per_2D_window" << symbols_per_2D_window <<"K_modulo" << K_modulo;
...
//	qDebug() << __FILE__ << __LINE__;
	...
	if (mode_and_occupancy_code != mode_and_occupancy_code_last)
	{
		...
		for (nnn = 0; nnn < y; nnn++)
		{
			...
			for (k_index1 = 0; k_index1 < (K_max - K_min + 1); k_index1++)
			{
				...
				//              qDebug() << "W1 nnn" << nnn << "j" << NP << "k" << k_index1;
				for (j = 0; j < NP; j++)
				{
			...
			for (k_index1 = 0; k_index1 < cnt_next_pilot_cells[nnn]; k_index1++)
			{
				...
				//              qDebug() << "W2 nnn" << nnn << "j" << NP << "k" << k_index1;
				for (j = 0; j < NP; j++)
				{
	...
	for (i = 0; i < symbols_per_frame; i++)
	{
		...
		for (j = 0; j < cnt_tr_cells[ntwee]; j++)
		{
			...
			//          logfile->addToAux(QString("hoek %1 %2 %3 %4  %5").arg(hoek).arg(tmpreal).arg(tmpimag).arg(symbol_buffer[2 * ntc_indx]).arg(symbol_buffer[2 * ntc_indx + 1]));

			if(ntc_indx >= (symbols_per_2D_window+1)*K_modulo)
			{
				//              qDebug() << symbol_buffer[2 * ntc_indx] << "ntc_indx" << ntc_indx;
				//              qDebug() << "ntc_indx" << ntc_indx;
			}
		}
		//      if ((iterationCounter>=0) && (iterationCounter<=180))
		//        {
		//          logfile->addToAux(QString("block %1").arg(iterationCounter));
		//          arrayDump("ntc1",gain_ref_cells_subset_nn,cnt_tr_cells[ntwee],true);
		//          arrayDump("ntc2", normalized_training_cells,cnt_tr_cells[ntwee]*2,true);
		//        }
		...
		for (j = 0; j < cnt_actual_pilots_rel_indx; j++)
		{
			...
			//          qDebug() << actual_pilots[2 * j];
			//          qDebug() << next_pilots[2 * j];
		}
		...
		//      qDebug() << "ntwee" << ntwee  << "k" <<cnt_tr_cells[ntwee] << "j" << cnt_actual_pilots_rel_indx;
		...
		for (j = 0; j < cnt_actual_pilots_rel_indx; j++)
		{
			...
			for (k = 0; k < cnt_tr_cells[ntwee]; k++)
			{
				...
				//              qDebug() << W_pilots_blk[ntwee][k][j];
				//              if((ntwee>2) || (k>45) ||(j>44))
				//                {
				//                  qDebug() << "*W3 nnn" << ntwee << "k" << k << "j" << j;
				//                }
			}
		}
		//      qDebug() << W_pilots_blk[0][45][0];
		//      qDebug() << W_pilots_blk[0][45][1];
		//      qDebug() << W_pilots_blk[0][45][2];
		//      qDebug() << W_pilots_blk[0][45][3];
		//     qDebug() << W_pilots_blk[0][45][4];
		//     qDebug() << W_pilots_blk[0][45][5];
		//     qDebug() << W_pilots_blk[0][45][6];
		//     qDebug() << W_pilots_blk[0][45][7];

		//     qDebug() << W_pilots_blk[1][45][0];
		//     qDebug() << W_pilots_blk[1][45][1];
		//     qDebug() << W_pilots_blk[1][45][2];
		//     qDebug() << W_pilots_blk[1][45][3];
		//    qDebug() << W_pilots_blk[1][45][4];
		//    qDebug() << W_pilots_blk[1][45][5];
		//    qDebug() << W_pilots_blk[1][45][6];
		//    qDebug() << W_pilots_blk[1][45][7];

		//    qDebug() << W_pilots_blk[2][45][0];
		//    qDebug() << W_pilots_blk[2][45][1];
		//    qDebug() << W_pilots_blk[2][45][2];
		//    qDebug() << W_pilots_blk[2][45][3];
		//   qDebug() << W_pilots_blk[2][45][4];
		//   qDebug() << W_pilots_blk[2][45][5];
		//   qDebug() << W_pilots_blk[2][45][6];
		//   qDebug() << W_pilots_blk[2][45][7];
	...
	//  qDebug()<< t_smp;
	//  qDebug()<< N_symbols_needed;
	//  qDebug() << Ts;
	//  qDebug() << smp_rate_conv_fft_phase_diff;
	//  qDebug() << smp_rate_conv_fft_phase_offset;
	//  qDebug() << smp_rate_conv_in_out_delay;
	...
	//  qDebug() << "smp_rate" <<smp_rate_conv_fft_phase_diff << smp_rate_conv_fft_phase_offset << smp_rate_conv_in_out_delay;
	//    display results
