...
char drmCallsign[9]= {0, 0, 0, 0, 0, 0, 0, 0, 0}; //changed joma
...
void channel_decoding(void)
{
  ...
  drmCallsign[3*identity] = getfacchar(&facblock[10]);
  drmCallsign[3*identity+1] = getfacchar(&facblock[17]);
  drmCallsign[3*identity+2] = getfacchar(&facblock[24]);
  drmCallsign[3*identity+3] = '\0';
  if ((identity == 2) && (identityCount>=3))
    {
      identityCount=0;
      callsignValid=true;
//						qDebug("callsign: %s",drmCallsign);
    }
